...
const mockGuidanceOnADRModel = {
  ...
  findOne: jest.fn().mockReturnValue({
    ...
    exec: jest
      .fn()
      .mockResolvedValue({ _id: '1', name: 'Test Guidance On ADR' }), // resolves the final query
  }),
  findById: jest
    .fn()
    .mockResolvedValue({ _id: '1', name: 'Test Guidance On ADR' }),
  findByIdAndUpdate: jest
    .fn()
    .mockResolvedValue({ _id: '1', title: 'Updated Guidance On ADR' }),
...
describe('AlgorithmGuidanceOnAdverseDrugReactionService', () => {
  let service: AlgorithmGuidanceOnAdverseDrugReactionService;
  ...
  beforeEach(async () => {
    ...
    service = module.get<AlgorithmGuidanceOnAdverseDrugReactionService>(
      AlgorithmGuidanceOnAdverseDrugReactionService,
    );
  });
  ...
  describe('create', () => {
    it('should create a Guidance on ADR Algorithm', async () => {
      const createGuidanceOnADRDto = {
        id: 1,
        nodeType: 'Node Type',
        isExpandable: false,
        hasOptions: false,
        ...
        isAllCadre: false,
        title: { en: 'Guidance on ADR Algorithm' },
        icon: 'icon',
        description: { en: 'Guidance on ADR Algorithm Description' },
        header: {},
        subHeader: {},
      ...
      const mockGuidanceOnADR = {
        _id: new mongoose.Types.ObjectId('6666c830eb18953046b1b56b'),
        ...createGuidanceOnADRDto,
      };
      mockGuidanceOnADRModel.create.mockResolvedValue(mockGuidanceOnADR);

      const result = await service.create(createGuidanceOnADRDto);
      expect(mockGuidanceOnADRModel.create).toHaveBeenCalledWith(
        createGuidanceOnADRDto,
      );
      expect(result).toEqual({
        statusCode: 200,
        message: 'Guidance On ADR Algorithm Created successfully',
        data: mockGuidanceOnADR,
      });
    });
  });
  ...
  describe('findAll', () => {
    it('should call filterService.filter and paginate with correct arguments', async () => {
      ...
      const mockGuidanceOnADRAlgo = [
        { name: 'Guidance on ADR Algo 1' },
        { name: 'Guidance on ADR Algo 2' },
      ];

      mockFilterService.filter.mockResolvedValue(mockQuery);
      (paginate as jest.Mock).mockResolvedValue(mockGuidanceOnADRAlgo);

      const result = await service.findAll(paginationDto);

      expect(mockFilterService.filter).toHaveBeenCalledWith(paginationDto);
      expect(paginate).toHaveBeenCalledWith(
        mockGuidanceOnADRModel,
        paginationDto,
        [
      ...
      expect(result).toEqual(mockGuidanceOnADRAlgo);
    });
  });

  describe('findOne', () => {
    it('should return a Guidance on ADR by ID', async () => {
      const mockGuidanceOnADR = { _id: '1', name: 'Test Guidance on ADR' };
      mockGuidanceOnADRModel.findById.mockReturnValue(mockGuidanceOnADR);

      const result = await service.findOne('1');
      expect(result).toEqual({
        statusCode: 200,
        message: 'Guidance On ADR Algorithm fetch successfully',
        data: mockGuidanceOnADR,
      });
      expect(mockGuidanceOnADRModel.findById).toHaveBeenCalledWith('1');
    });

    it('should return null if Guidance on ADR not found', async () => {
      mockGuidanceOnADRModel.findById.mockResolvedValueOnce(null);
  ...
  describe('update', () => {
    it('should update and return the updated Guidance on ADR', async () => {
      const updatedGuidanceOnADR = {
        _id: '1',
        nodeType: 'Updated Guidance on ADR',
      };
      mockGuidanceOnADRModel.findByIdAndUpdate.mockResolvedValue(
        updatedGuidanceOnADR,
      );
      ...
      expect(result).toEqual({
        statusCode: 200,
        message: 'Guidance On ADR Algorithm updated successfully',
        data: updatedGuidanceOnADR,
      });
      expect(mockGuidanceOnADRModel.findByIdAndUpdate).toHaveBeenCalledWith(
        '1',
        { nodeType: 'Updated Guidance on ADR' },
        { new: true },
      );
  ...
  describe('remove', () => {
    it('should delete a Guidance on ADR Algorithm by ID', async () => {
      mockGuidanceOnADRModel.findByIdAndDelete.mockResolvedValueOnce({});

      const result = await service.remove('1');

      expect(mockGuidanceOnADRModel.findByIdAndDelete).toHaveBeenCalledWith(
        '1',
      );
      expect(result).toEqual({
        statusCode: 200,
        message: 'Guidance On ADR Algorithm deleted successfully',
        data: [],
      });
    });
  });
  ...
  describe('getChild', () => {
    it('should return child nodes when rootNode is found', async () => {
      ...
      mockGuidanceOnADRModel.findById.mockReturnValue({
        populate: jest.fn().mockReturnThis(),
      ...
      expect(mockGuidanceOnADRModel.findById).toHaveBeenCalledWith(rootNodeId);
  ...
  describe('getMasterNodes', () => {
    it('should return master nodes with status 200', async () => {
      ...
      mockGuidanceOnADRModel.find.mockReturnValue({
        exec: jest.fn().mockResolvedValue(mockNodes),
      });

      const result = await service.getMasterNodes();

      expect(mockGuidanceOnADRModel.find).toHaveBeenCalledWith({
        parentId: null,
      });
  ...
  describe('getMasterNode', () => {
    it('should return master nodes for subscriber with stateId', async () => {
      ...
      mockGuidanceOnADRModel.find.mockReturnValueOnce({
        exec: jest.fn().mockResolvedValue(mockNodes),
      });

      const result = await service.getMasterNode('userId123', 'en');
      expect(mockGuidanceOnADRModel.find).toHaveBeenCalled();
  ...
  describe('getAllDescendants', () => {
    it('should return all root nodes with their descendants', async () => {
      ...
      mockGuidanceOnADRModel.findById.mockImplementation(() => ({
        populate: jest.fn().mockReturnThis(),
      ...
      expect(mockGuidanceOnADRModel.findById).toHaveBeenCalledTimes(2);
  ...
  describe('sendInitialInvitation', () => {
    const mockGuidanceOnADRId = '6666c830eb18953046b1b56b';
    ...
    it('should add notification to queue and return response', async () => {
      mockGuidanceOnADRModel.findById.mockResolvedValue({
        _id: mockGuidanceOnADRId,
        parentId: null,
        title: { en: 'Guidance on adr Title' },
        isAllCadre: false,
      ...
      expect(mockGuidanceOnADRModel.findById).toHaveBeenCalledWith(
        mockGuidanceOnADRId,
      );
    ...
    it('should update diagnosis when no device tokens are found', async () => {
      ...
      await service.sendInitialInvitation(mockGuidanceOnADRId, mockAdminUserId);
