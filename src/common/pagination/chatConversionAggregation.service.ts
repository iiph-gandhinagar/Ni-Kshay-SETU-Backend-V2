...
export async function ChatConversionAggregation(
  model: Model<any>,
  paginationDto: any,
  filters: any = {},
) {
  ...
  const aggregationPipeline: any[] = [
    ...
    { $match: filters },
    {
      $set: {
        message: {
          $sortArray: { input: '$message', sortBy: { createdAt: -1 } }, // Sort descending (latest first)
        },
      },
    },

    // Step 2: Extract the latest `createdAt` from the first element of `message`
    {
      $set: {
        createdAt: { $arrayElemAt: ['$message.createdAt', 0] },
      },
    },

    // Step 3: Convert `createdAt` to a proper date format (optional)
    {
      $set: {
    ...
    },
    {
      $project: {
        ...
        createdAt: {
          $dateToString: {
            format: '%Y-%m-%d %H:%M:%S', // Format `createdAt`
            date: '$createdAt',
  ...
  if (country) nonNullChecks['userId.country'] = { $ne: null };
  if (stateIds) nonNullChecks['userId.state'] = { $ne: null };
  if (userCadreId) nonNullChecks['userId.cadre'] = { $ne: null };
  if (districtIds) nonNullChecks['userId.district'] = { $ne: null };
